...
class TRAINER(object):
    ...
    def f_train_epoch(self, data, network, optimizer, logger_obj, train_val_flag):
        ...
        for input_batch, user_batch, target_batch, ARe_batch, RRe_batch, length_batch in data:
            ...
            logits, s_mean, s_logv, s = network(input_batch, user_batch, length_batch)
            ...
            ### ARe loss
            ARe_loss = self.m_ARe_loss_fn(logits, ARe_batch)
            ARe_loss = ARe_loss/batch_size

            # loss = ARe_loss
            loss = ARe_loss+KL_weight_s*KL_loss_s
            ...
            if iteration % self.m_print_interval == 0:
                logger_obj.f_add_output2IO("%04d, Loss %9.4f, KL-S_Loss %.4f, KL-Weight_s %.4f,  ARe_loss %.4f"
                    %(iteration, np.mean(loss_list), np.mean(KL_loss_s_list), KL_weight_s, np.mean(ARe_loss_list)))
