...
from batchify_vae import get_batches
...
def evaluate(model, batches):
    ...
    with torch.no_grad():
        for inputs, targets, lengths in batches:
            losses = model(inputs, lengths, targets, False)
...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        for i, idx in enumerate(indices):
            inputs, targets, lengths = train_batches[idx]

            losses = model(inputs, lengths, targets, is_train=True)
