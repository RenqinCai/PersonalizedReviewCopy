...
class DAE(TextModel):
    ...
    def __init__(self, vocab, args):
        ...
        self.E = nn.GRU(args.dim_emb, args.dim_h, args.nlayers,
            dropout=args.dropout if args.nlayers > 1 else 0, bidirectional=True)
        self.G = nn.GRU(args.dim_emb, args.dim_h, args.nlayers,
            dropout=args.dropout if args.nlayers > 1 else 0)
        ...
        # self.z2emb = nn.Linear(args.dim_z, args.dim_h)
    ...
    def decode(self, z, input, hidden=None):
        ...
        # print(self.embed(input).size())
        # print("z", self.z2emb(z).size())
        # input = self.drop(self.embed(input)) 
        # input = self.drop(self.embed(input))
        # init_hidden = self.z2emb(z)
        # hidden = init_hidden.unsqueeze(0)
        # print("input", input.size())
        # print("hidden", hidden.size())
        # init_hidden = init_hidden.expand(input.size())
