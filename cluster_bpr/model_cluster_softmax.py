...
class _ATTR_NETWORK(nn.Module):
    def __init__(self, vocab_obj, args, device):
        ...
        self.m_cluster_embedding_user = nn.Parameter(torch.randn(cluster_num, self.m_attr_embed_size))
        self.m_cluster_embedding_item = nn.Parameter(torch.randn(cluster_num, self.m_attr_embed_size))
        ...
        self.m_output_attr_embedding_user = self.m_attr_embedding_user.weight
        self.m_output_attr_embedding_item = self.m_attr_embedding_item.weight
    ...
    def f_init_weight(self):
        ...
        torch.nn.init.uniform_(self.m_cluster_embedding_user, -initrange, initrange)
        torch.nn.init.uniform_(self.m_cluster_embedding_item, -initrange, initrange)
    ...
    def forward(self, attr_item, attr_tf_item, attr_lens_item, item_ids, attr_user, attr_tf_user, attr_lens_user, user_ids):
        ...
        cluster_weight_item = torch.matmul(attr_embed_item, self.m_cluster_embedding_item.t())
        ...
        ### cluster_weight_item: batch_size*max_len*cluster_num
        cluster_weight_user = torch.matmul(attr_embed_user, self.m_cluster_embedding_user.t())
        ...
        attr_embed_user = self.m_output_attr_embedding_user

        attr_embed_item = self.m_output_attr_embedding_item
