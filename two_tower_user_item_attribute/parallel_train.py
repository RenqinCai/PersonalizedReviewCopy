...
class _TRAINER(object):
    ...
    def f_train(self, pretrain_word_embed, train_data, eval_data, network, optimizer, logger_obj, local_rank):
        last_train_loss = 0
        last_eval_loss = 0
    ...
    def f_train_epoch(self, train_data, network, optimizer, logger_obj):
        ...
        for attr_item_batch, attr_tf_item_batch, attr_length_item_batch, item_batch, attr_user_batch, attr_tf_user_batch, attr_length_user_batch, user_batch, pos_target_batch, pos_length_batch, neg_target_batch, neg_length_batch in train_data:
            ...
            logits, mask, targets = network.module.f_pred_forward(output, pos_target_gpu, pos_length_gpu, neg_target_gpu, neg_length_gpu)
    ...
    def f_eval_epoch(self, eval_data, network, optimizer, logger_obj):
        ...
        with torch.no_grad():
            for attr_item_batch, attr_tf_item_batch, attr_length_item_batch, item_batch, attr_user_batch, attr_tf_user_batch, attr_length_user_batch, user_batch, target_batch, target_mask_batch in eval_data:			
                ...
                logits = network.module.f_eval_forward(output)
