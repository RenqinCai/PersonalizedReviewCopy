...
def evaluate(args, model_vae, decoder_tokenizer, table_name, prefix="", subset="test"):
    ...
    eval_dataloader = build_dataload_and_cache_examples(args, [decoder_tokenizer], evaluate=True)
...
def evaluate_rec(args, model_vae, decoder_tokenizer, table_name, prefix="", subset="test"):
    # Loop to handle MNLI double evaluation (matched, mis-matched)
    eval_output_dir = args.output_dir
    ...
    if subset == 'test':
        ...
        eval_dataloader = build_dataload_and_cache_examples(args, [ecoder_tokenizer], evaluate=False)
    logger.info("***** Running evaluation on {} dataset *****".format(subset))
...
def main():
    ...
    if args.do_train:
        ...
        print("==="*20)
        train_dataloader = build_dataload_and_cache_examples(args, [tokenizer_decoder], evaluate=False)
        print("==="*20)
        ...
        global_step, tr_loss, optimizer = train(args, train_dataloader, model_vae, tokenizer_decoder, table_name)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        result = evaluate(args, model_vae, tokenizer_decoder, table_name, prefix=global_step, subset='test')
    ...
    if args.do_eval_rec and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model_vae = VAE(model_decoder, tokenizer_decoder, args).to(args.device)

            result = evaluate_rec(args, model_vae, tokenizer_decoder, table_name, prefix=global_step, subset='test')
            result = dict((k + '_test_{}'.format(global_step), v) for k, v in result.items())
            results.update(result)

            result = evaluate_rec(args, model_vae, tokenizer_decoder, table_name, prefix=global_step, subset='train')
