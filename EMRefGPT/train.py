...
class _TRAINER(object):
	...
	def __init__(self, vocab, args, device):
		...
		self.m_num_train_epochs = args.num_train_epochs
		...
		self.m_max_steps = args.max_steps
	...
	def f_train_M(self, train_data, eval_data, E_network, network, optimizer, scheduler, logger_obj, rank, args):
		print("----"*10)

		# network.f_init_user_item_word(E_network)
		# network.f_init_user_item_word(E_network)
		
		train_iterator = trange(int(self.m_num_train_epochs), desc='Epoch', disable=False)

		n_iter = int(self.m_num_train_epochs)*len(train_data)
		tmp_list = []
		...
		for epoch in train_iterator:
			epoch_iterator = tqdm(train_data, desc='Iteration', disable=False)
			for step, batch in enumerate(epoch_iterator):
				userid, itemid, tokenized_text, tokenized_text_length = batch

				labels = tokenized_text
				tokenized_text = tokenized_text.to(self.m_device)
				...
				# global_step += 1
				# if global_step > self.m_max_steps:
	...
	# def f_save_checkpoint(model, optimizer, args):
	# 	output_decoder_dir = os.path.join(args.output_dir, "checkpoint-decoder-best")
	# 	if not os.path.exists(output_decoder_dir) and args.local_rank in [-1, 0]:
	# 		os.makedirs(output_decoder_dir)

	# 	model_decoder_to_save = model.m_decoder
	...
	# 	output_full_dir = os.path.join(args.output_dir, 'checkpoint-full-best')
	...
	def f_train_de_epoch(self, train_data, network, de_optimizer, logger_obj):
		...
		for input_batch, input_length_batch, user_batch, item_batch, target_batch, target_length_batch, random_flag in train_data:
			...
			logits = network(input_batch_gpu, user_batch_gpu, item_batch_gpu, random_flag)
