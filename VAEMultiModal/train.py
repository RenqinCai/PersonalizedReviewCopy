...
class TRAINER(object):
    ...
    def f_train_epoch(self, data, network, optimizer, logger_obj, train_val_flag):
        ...
        for input_batch, target_batch, length_batch in data:
            ...
            logp, z_mean_pool, z_logv_pool, last_en_hidden, hidden_pool = network(input_batch, length_batch)
            ...
            KL_loss_z_multi = []
            KL_weight_z_multi = []
            for z_mean, z_logv in zip(*(z_mean_pool, z_logv_pool)):
                KL_loss_z, KL_weight_z = self.m_KL_loss_z_fn(z_mean, z_logv, self.m_train_step)
                KL_loss_z_multi.append(KL_loss_z*KL_weight_z)
                # print("KL_weight_z", KL_weight_z)
                KL_weight_z_multi.append(KL_weight_z)

            KL_loss_z = torch.mean(torch.stack(KL_loss_z_multi), dim=0)
            self.m_KL_weight = np.mean(KL_weight_z_multi)
            # KL_loss_z = torch.mean(KL_loss_z_multi)
