...
class REVIEWDI(nn.Module):
    def __init__(self, vocab_obj, args, device):
        ...
        self.m_hidden2mean_z_list = [nn.Linear(self.m_hidden_size*self.m_hidden_factor, self.m_latent_size).to(self.m_device) for i in range(self.m_mode_num)]

        self.m_hidden2logv_z_list = [nn.Linear(self.m_hidden_size*self.m_hidden_factor, self.m_latent_size).to(self.m_device) for i in range(self.m_mode_num)]

        self.m_latent2hidden_list = [nn.Linear(self.m_latent_size, self.m_hidden_size*self.m_hidden_factor).to(self.m_device) for i in range(self.m_mode_num)]
    ...
    def forward(self, input_sequence, length):
        ...
        encoder_outputs, _ = self.m_encoder_rnn(packed_input)
        ...
        z_mean_pool = []
        z_logv_pool = []
        for i in range(self.m_mode_num):
            z_mean = self.m_hidden2mean_z_list[i](last_en_hidden)
            z_logv = self.m_hidden2logv_z_list[i](last_en_hidden)
            z_std = torch.exp(0.5*z_logv)
            pre_z = torch.randn([batch_size, self.m_latent_size]).to(self.m_device)

            z = pre_z*z_std + z_mean
            hidden = self.m_latent2hidden_list[i](z)

            z_mean_pool.append(z_mean)
            z_logv_pool.append(z_logv)

            hidden = hidden.unsqueeze(1)
            hidden_pool.append(hidden)

        hidden_pool = torch.cat(hidden_pool, dim=1)
        ...
        alignment_scores = self.m_attn(padded_outputs, hidden_pool)
        attn_weights = F.softmax(alignment_scores, dim=-1)
        context_vector = torch.bmm(attn_weights, hidden_pool)
        ...
        return logp, z_mean_pool, z_logv_pool, last_en_hidden, hidden_pool
