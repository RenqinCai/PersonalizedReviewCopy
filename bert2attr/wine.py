...
class _WINE(Dataset):
    def __init__(self, args, vocab_obj, df, boa_item_dict, boa_user_dict):
        ...
        self.m_attr_batch_list = []
        self.m_attr_ind_batch_list = []
        self.m_attr_tf_batch_list = []
        self.m_attr_feat_batch_list = []
        self.m_attr_length_list = []
        ...
        for sample_index in range(self.m_sample_num):
            ...
            attr_ind_i = [1 for i in range(len(attrlist_user_i))]+[2 for i in range(len(attrlist_item_i))]
            self.m_attr_ind_batch_list.append(attr_ind_i)

            attr_i = attrlist_user_i+attrlist_item_i
            self.m_attr_batch_list.append(attr_i)

            attr_tf_i = attrfreq_list_user_i+attrfreq_list_item_i
            self.m_attr_tf_batch_list.append(attr_tf_i)

            self.m_attr_length_list.append(len(attr_i))
            
            ### mean_user, var_user, min_user, max_user, 
            # mean_item, var_item, min_item, max_item, 
            # number of features user, number of features item, user > item, max overall, min_overall
            feature_i = [np.mean(attrfreq_list_user_i), np.sqrt(np.var(attrfreq_list_user_i)), np.min(attrfreq_list_user_i), np.max(attrfreq_list_user_i), \
                np.mean(attrfreq_list_item_i), np.sqrt(np.var(attrfreq_list_item_i)), np.min(attrfreq_list_item_i), np.max(attrfreq_list_item_i), \
                len(attrfreq_list_user_i), len(attrfreq_list_item_i), (attrfreq_list_item_i[0] > attrfreq_list_user_i[0])*1.0, np.max(attr_i), np.min(attr_i)]
            self.m_attr_feat_batch_list.append(feature_i)
    ...
    def __getitem__(self, idx):
        ...
        attr_i = self.m_attr_batch_list[i]
        attr_ind_i = self.m_attr_ind_batch_list[i]

        attr_tf_i = self.m_attr_tf_batch_list[i]
        attr_feat_i = self.m_attr_feat_batch_list[i]
        attr_length_i = self.m_attr_length_list[i]
        ...
        sample_i = {"attr": attr_i, "attr_ind": attr_ind_i, "attr_tf": attr_tf_i, "attr_feat": attr_feat_i, "attr_length": attr_length_i, "attr_length_user": attr_length_user_i, "attr_length_item": attr_length_item_i,  "user": user_i,  "item": item_i, "pos_target": pos_target_i, "pos_len": pos_len_i, "neg_target": neg_target_i, "neg_len": neg_len_i}
    ...
    def collate(batch):
        ...
        for i in range(batch_size):
            ...
            attr_length_i = sample_i["attr_length"]
        ...
        for i in range(batch_size):
            ...
            attr_i = [int(i) for i in attr_i]

            attr_length_i = sample_i["attr_length"]
            
            attr_i.extend([pad_id]*(max_attr_length_iter-attr_length_i))
            attr_iter.append(attr_i)

            attr_ind_i = copy.deepcopy(sample_i["attr_ind"])
            attr_ind_i.extend([ind_pad_id]*(max_attr_length_iter-attr_length_i))
            attr_ind_iter.append(attr_ind_i)

            attr_tf_i = copy.deepcopy(sample_i['attr_tf'])
            attr_tf_i.extend([freq_pad_id]*(max_attr_length_iter-attr_length_i))
            attr_tf_iter.append(attr_tf_i)
            ...
            attr_feat_i = sample_i["attr_feat"]
            attr_feat_iter.append(attr_feat_i)
        ...
        attr_ind_iter_tensor = torch.from_numpy(np.array(attr_ind_iter)).long()

        attr_tf_iter_tensor = torch.from_numpy(np.array(attr_tf_iter)).float()
        attr_feat_iter_tensor = torch.from_numpy(np.array(attr_feat_iter)).float()
        ...
        return attr_iter_tensor, attr_ind_iter_tensor, attr_tf_iter_tensor, attr_feat_iter_tensor, attr_length_iter_tensor, attr_length_user_iter_tensor, attr_length_item_iter_tensor, user_iter_tensor, item_iter_tensor, pos_target_iter_tensor, pos_len_iter_tensor, neg_target_iter_tensor, neg_len_iter_tensor
...
class _WINE_TEST(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sample_i = {"attr": attr_i, "attr_ind": attr_ind_i, "attr_tf": attr_tf_i, "attr_feat": attr_feat_i, "attr_length": attr_length_i, "attr_length_user": attr_length_user_i, "attr_length_item": attr_length_item_i,  "user": user_i,  "item": item_i,  "target": target_i, "target_len": target_len_i}
    ...
    def collate(batch):
        ...
        return attr_iter_tensor, attr_ind_iter_tensor, attr_tf_iter_tensor, attr_feat_iter_tensor, attr_length_iter_tensor, attr_length_user_iter_tensor, attr_length_item_iter_tensor, user_iter_tensor, item_iter_tensor, target_iter_tensor, target_mask_iter_tensor
