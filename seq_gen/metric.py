...
def get_precision_recall_F1_train(preds, targets, lens, k=1):
    ...
    for i, pred_index in enumerate(preds):

        len_i = lens[i].item()

        pred_i = list(pred_index.numpy())
        pred_i = pred_i[:len_i-1]
        # print("pred_i", pred_i)
        target_i = list(targets[i, :len_i-1].numpy())
        ...
        precision = true_pos_num/pred_num_i
        recall = true_pos_num/(len_i-1+1e-26)
...
def get_precision_recall_F1(preds, targets, lens, k=1):
    ...
    for i, _ in enumerate(preds):
        ...
        print(preds[i][0])
        print("target_i", target_i)
        print(targets[i, :len_i])

        pred_num_i = len(pred_i)
        print("pred_num_i", pred_num_i)
