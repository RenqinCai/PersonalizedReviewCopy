"""
use transformer to get the score
...
class _ATTR_NETWORK(nn.Module):
    ...
    def forward(self, pos_attr_set, pos_attr_lens, neg_attr_set, neg_attr_lens, neg_attr_set_num, user_ids, item_ids, _):
        ...
        pos_attr_set_user = self.m_set_enc(pos_attr_embed_user, pos_attr_mask)
        ...
        pos_user_attr_set_score = torch.sum(pos_user_attr_set_score*(~pos_attr_mask.unsqueeze(-1)), dim=1)        # print("pos_user_attr_set_score", pos_user_attr_set_score.size())
    ...
    def f_greedy_decode(self, user_ids, item_ids, pre_attrs, targets, step_index):
        ...
        # ### user_score: batch_size*attr_size*1
    ...
    def f_get_score(self, batch_size, user, attr, targets, pre_attr):
        ...
        next_input_attr_attn = self.m_set_enc(next_input_attr, None)

        ### next_input_attr: (batch_size*attr_size-seq_len)*embed_size
        next_input_attr_attn = torch.sum(next_input_attr_attn, dim=1)

        ### next_input_attr_attn: batch_size*(attr_size-seq_len)*embed_size
        next_input_attr_attn = next_input_attr_attn.reshape(batch_size, -1, next_input_attr_attn.size(-1))

        ### user_score: batch_size*(attr_size-seq_len)*1
        user_score = torch.matmul(next_input_attr_attn, user.unsqueeze(-1))

        ### user_score: batch_size*(attr_size-seq_len)
        user_score = user_score.squeeze(-1)
