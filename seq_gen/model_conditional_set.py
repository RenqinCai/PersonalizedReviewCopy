...
use the most popular attribute to condition prediction
...
class _ATTR_NETWORK(nn.Module):
    ...
    def f_init_weight(self):
        ...
        self.m_output_attr_embedding_x.apply(init_weights)
    ...
    def forward(self, pos_attr_set, pos_attr_lens, neg_attr_set, neg_attr_lens, neg_attr_set_num, user_ids, item_ids, first_flags):
        ...
        pos_attr_embed_x = self.m_output_attr_embedding_x(pos_attr_set)
        ...
        neg_attr_embed_x = self.m_output_attr_embedding_x(neg_attr_set)
        ...
        pos_attr_cond_score = torch.matmul(pos_attr_embed_x*(~pos_attr_mask.unsqueeze(-1)), pos_attr_embed_x[:, 0, :].unsqueeze(-1))
        pos_attr_cond_score = torch.sum((pos_attr_cond_score*(~pos_attr_mask.unsqueeze(-1)))[:, 1:], dim=1)
        ...
        ### sum these two 
        pos_attr_set_cond_score = pos_user_attr_set_score + pos_item_attr_set_score + pos_attr_cond_score*(~first_flags.unsqueeze(-1))

        pos_attr_set_score = pos_user_attr_set_score + pos_item_attr_set_score
        #  + pos_user_attr_cond_score + pos_item_attr_cond_score
        ...
        neg_attr_cond_score = torch.matmul(neg_attr_embed_x*(~neg_attr_mask.unsqueeze(-1)), neg_attr_embed_x[:, 0, :].unsqueeze(-1))
        neg_attr_cond_score = torch.sum((neg_attr_cond_score*(~neg_attr_mask.unsqueeze(-1)))[:, 1:], dim=1)
        ...
        neg_first_flags = first_flags.repeat_interleave(neg_attr_set_num, dim=0)
        neg_attr_set_cond_score = neg_user_attr_set_score + neg_item_attr_set_score + neg_attr_cond_score*(~neg_first_flags.unsqueeze(-1))

        neg_attr_set_score = neg_user_attr_set_score + neg_item_attr_set_score

        
        # print("neg_attr_set_score", neg_attr_set_score.size())
        # + neg_user_attr_cond_score + neg_item_attr_cond_score

        dup_pos_attr_set_cond_score = pos_attr_set_cond_score.repeat_interleave(neg_attr_set_num, dim=0)

        dup_pos_attr_set_score = pos_attr_set_score.repeat_interleave(neg_attr_set_num, dim=0)

        pos_attr_set_score

        cond_logits = (dup_pos_attr_set_cond_score-neg_attr_set_cond_score) - (dup_pos_attr_set_score-neg_attr_set_score)

        logits = dup_pos_attr_set_score-neg_attr_set_score

        # print("dup_pos_attr_set_score", dup_pos_attr_set_score.size())

        # print("logits", logits.size())

        return cond_logits, logits, pos_attr_mask
    ...
    def f_greedy_decode(self, user_ids, item_ids, pre_attrs, targets, step_index):
        ...
        x_score = 0
        if step_index > 0:
            ### attr_x: attr_size*embed_size
            attr_x = self.m_output_attr_embedding_x.weight.data

            ### pre_attr_x: batch_size*seq_len*embed_size
            pre_attr_x = self.m_output_attr_embedding_x(pre_attrs)

            ### pre_attr_x[:, 0]: batch_size*embed_size
            x_score = torch.matmul(pre_attr_x[:, 0], attr_x.t())

            ### x_score: batch_size*attr_size
            x_score = x_score[~targets]
            x_score = x_score.reshape(batch_size, -1)
        ...
        score = user_score+item_score+x_score
