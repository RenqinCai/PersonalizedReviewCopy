...
class _TRAINER(object):
    ...
    def f_train_epoch(self, train_data, network, optimizer, logger_obj):
        ...
        for ref_attr_item_batch, ref_attr_mask_item_batch, ref_item_batch, ref_item_mask_batch, user_batch, item_batch, pos_target_batch, pos_length_batch, neg_target_batch, neg_length_batch in train_data:
            
            ref_attr_item_gpu = ref_attr_item_batch.to(self.m_device)
            ref_attr_mask_item_gpu = ref_attr_mask_item_batch.to(self.m_device)

            ref_item_gpu = ref_item_batch.to(self.m_device)
            ref_item_mask_gpu = ref_item_mask_batch.to(self.m_device)
            ...
            logits, mask, targets = network(ref_attr_item_gpu, ref_attr_mask_item_gpu, ref_item_gpu, ref_item_mask_gpu, user_gpu, item_gpu, pos_target_gpu, pos_length_gpu, neg_target_gpu, neg_length_gpu)
    ...
    def f_eval_epoch(self, eval_data, network, optimizer, logger_obj):
        ...
        network.eval()
        with torch.no_grad():
            for ref_attr_item_batch, ref_attr_mask_item_batch, ref_item_batch, ref_item_mask_batch,  user_batch, item_batch, target_batch, target_mask_batch in eval_data:			
                ref_attr_item_gpu = ref_attr_item_batch.to(self.m_device)
                ref_attr_mask_item_gpu = ref_attr_mask_item_batch.to(self.m_device)
                ...
                logits = network.f_eval_forward(ref_attr_item_gpu, ref_attr_mask_item_gpu, ref_item_gpu, ref_item_mask_gpu, user_gpu, item_gpu)
