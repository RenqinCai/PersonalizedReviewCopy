...
class _ATTR_NETWORK(nn.Module):
    ...
    def forward(self, attr, attr_inds, attr_tf, attr_feat, attr_lens, attr_lens_user, attr_lens_item, user_ids, item_ids, pos_targets, pos_lens, neg_targets, neg_lens):
        ...
        for i in range(batch_size):
            ...
            # attr_tf_i = attr_tf[i]
            # attr_attn_i = attr_attn[i]
            
            attr_user_weight_i = attr_user_weight[i]
            attr_user_tf_weight_i = attr_user_weight_i*attr_tf[i][:attr_lens_user_i]
            tmp_user = attr_attn[i][:attr_lens_user_i]*attr_user_tf_weight_i

            attr_item_weight_i = attr_item_weight[i]
            attr_item_tf_weight_i = attr_item_weight_i*attr_tf[i][attr_lens_user_i:attr_lens_user_i+attr_lens_item_i]
            tmp_item = attr_attn[i][attr_lens_user_i:attr_lens_user_i+attr_lens_item_i]*attr_item_tf_weight_i

            avg_attr_user_i = tmp_user.sum(0)/attr_lens_user_i
            avg_attr_item_i = tmp_item.sum(0)/attr_lens_item_i

            # avg_attr_user_i = tmp_user.sum(0)/(attr_user_tf_weight_i.sum()+1e-20)
            # # print(attr_user_tf_weight_i.sum())

            # avg_attr_item_i = tmp_item.sum(0)/(attr_item_tf_weight_i.sum()+1e-20)
            # print("attr_tf_i[attr_lens_user_i:attr_lens_user_i+attr_lens_item_i]", attr_tf_i[attr_lens_user_i:attr_lens_user_i+attr_lens_item_i])
            # print("attr_item_weight_i", attr_item_weight_i)
            # print("item", attr_item_tf_weight_i.sum())
    ...
    def f_eval_forward(self, attr, attr_inds, attr_tf, attr_feat, attr_lens, attr_lens_user, attr_lens_item, user_ids, item_ids):
        ...
        for i in range(batch_size):
            ...
            ### seqlen*1
            # print("attr_item_tf_weight_i", attr_item_tf_weight_i.size())

            # avg_attr_user_i = tmp_user.sum(0)/(attr_user_tf_weight_i.sum())
            # print(attr_user_tf_weight_i.sum())

            # avg_attr_item_i = tmp_item.sum(0)/(attr_item_tf_weight_i.sum())
            # print(attr_item_tf_weight_i.sum())
