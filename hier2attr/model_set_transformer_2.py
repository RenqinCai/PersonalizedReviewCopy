...
class _ATTR_NETWORK(nn.Module):
    ...
    def f_get_x(self, attr, attr_lens, item_lens, user_item_flag):
        print("***"*10)
        print("attr", attr)
        ...
        if user_item_flag == "user":
            attr = self.m_output_attr_embedding_user(attr)
        
        if user_item_flag == "item":
            attr = self.m_output_attr_embedding_item(attr)
        ...
        # print("attr_w", attr_w.size())
        print("attr_w", len(attr_w))
        for i in range(len(attr_w)):
            print("--"*10)
            print(attr_w[i].size())

            sample_num = attr_w[i].size(0)
            attr_num = attr_w[i].size(1)
            for j in range(sample_num):
                for k in range(attr_num):
                    for l in range(attr_num):
                        print(attr_w[i][j][k][l].item(), end=", ")
                    print()
                print()
        ...
        # print("user_w", len(user_w))

        # for i in range(len(user_w)):
        #     print("--"*10)
        #     print(user_w[i].size())

        # print("user_w", user_w.size())
        exit()
        return attr_x
    ...
    def forward(self, ref_attr_item_user, ref_attr_len_item_user, ref_item_len_user, ref_attr_user_item, ref_attr_len_user_item, ref_user_len_item, user_ids, item_ids, pos_targets, pos_lens, neg_targets, neg_lens):
        ...
        user_x = self.f_get_x(ref_attr_item_user, ref_attr_len_item_user, ref_item_len_user, "user")

        item_x = self.f_get_x(ref_attr_user_item, ref_attr_len_user_item, ref_user_len_item, "item")
