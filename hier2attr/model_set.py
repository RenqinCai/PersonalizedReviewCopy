...
class _ATTR_NETWORK(nn.Module):
    def __init__(self, vocab_obj, args, device):
        ...
        self.m_enc = nn.Sequential(
            ...
            nn.Linear(in_features=64, out_features=64),
            nn.ReLU(),
            nn.Linear(in_features=64, out_features=self.m_attr_embed_size),
        )
        ...
        # self.m_set_weigt = nn.Parameter(torch.Tensor(self.m_attr_embed_size, self.m_attr_embed_size))
        # self.m_set_bias = nn.Parameter(torch.Tensor(1, self.m_attr_embed_size))
        self.m_set_linear = nn.Linear(self.m_attr_embed_size, self.m_attr_embed_size)
        self.m_set_context = nn.Linear(self.m_attr_embed_size, 1, bias=False)
        
        # nn.Parameter(torch.Tensor(self.m_attr_embed_size, 1))
    ...
    def f_init_weight(self):
        ...
        # torch.nn.init.uniform_(self.m_set_linear.weight, -initrange, initrange)

        # torch.nn.init.uniform_(self.m_set_context.weight, -initrange, initrange)
    ...
    def f_get_avg_attr_user(self, attr_item, item_lens, flag):  
        ...
        same_user_simlist_list = []

        for i in range(attr_user.size(0)):
            attr_user_i = attr_user[i]
            simlist = []
            for j in range(1, attr_user.size(1)):
                attr_user_ij_pre = attr_user_i[j-1].unsqueeze(0)
                attr_user_ij = attr_user_i[j].unsqueeze(0)

                sim_ij = F.cosine_similarity(attr_user_ij_pre, attr_user_ij)
                simlist.append(round(sim_ij.item(), 4))

            same_user_simlist_list.append(simlist)
        
        print("same user simlist_list", same_user_simlist_list)

        diff_user_simlist_list = []

        for i in range(attr_user.size(1)):
            # attr_user_i = attr_user[i]
            simlist = []
            for j in range(1, attr_user.size(0)):
                attr_user_ij_pre = attr_user[j-1][i].unsqueeze(0)
                attr_user_ij = attr_user[j][i].unsqueeze(0)

                sim_ij = F.cosine_similarity(attr_user_ij_pre, attr_user_ij)
                simlist.append(round(sim_ij.item(), 4))

            diff_user_simlist_list.append(simlist)
        print("diff_user_simlist_list", diff_user_simlist_list)
