...
class _WINE(Dataset):
    def __init__(self, args, vocab_obj, df, boa_item_dict, boa_user_dict):
        ...
        ### for item
        self.m_ref_attr_user_batch_list = []
        self.m_ref_user_batch_list = []
        ...
        user_itemdict_attrlist_dict, item_userdict_attrlist_dict = self.f_split(df)
        ...
        for sample_index in range(self.m_sample_num):
            ...
            userdict_attrlist_dict = item_userdict_attrlist_dict[itemid_i]
            for userid_j in userdict_attrlist_dict:
                if userid_j == userid_i:
                    continue

                attrlist_j = userdict_attrlist_dict[userid_j]
    ...
    def f_split(self, df):
        ...
        for (userid_i, itemid_i) in user_item_attrlist_dict:
            ...
            if itemid_i not in item_userdict_attrlist_dict:
                item_userdict_attrlist_dict[itemid_i] = {}
            
            if userid_i not in item_userdict_attrlist_dict[itemid_i]:
                item_userdict_attrlist_dict[itemid_i][userid_i] = []

            item_userdict_attrlist_dict[itemid_i][userid_i] = attr_i
        ...
        reserve_item_userdict_attrlist_dict = {}
        maxusernum_item = maxitemnum_user
        for itemid_i in item_userdict_attrlist_dict:
            userdict_attrlist_dict_i = item_userdict_attrlist_dict[itemid_i]

            userid_list_i = list(userdict_attrlist_dict_i.keys())
            random.shuffle(userid_list_i)

            reserve_usernum = len(userid_list_i)
            if reserve_usernum > maxusernum_item:
                reserve_usernum = maxusernum_item
            
            if itemid_i not in reserve_item_userdict_attrlist_dict:
                reserve_item_userdict_attrlist_dict[itemid_i] = {}
            
            for j in range(reserve_usernum):
                userid_ij = userid_list_i[j]
                attr_ij = userdict_attrlist_dict_i[userid_ij]
                if userid_ij not in reserve_item_userdict_attrlist_dict[itemid_i]:
                    reserve_item_userdict_attrlist_dict[itemid_i][userid_ij] = []
                reserve_item_userdict_attrlist_dict[itemid_i][userid_ij] = attr_ij

        return reserve_user_itemdict_attrlist_dict, reserve_item_userdict_attrlist_dict
    ...
    def __getitem__(self, idx):
        ...
        ref_attr_user_list_i = self.m_ref_attr_user_batch_list[i]

        ref_user_list_i = self.m_ref_user_batch_list[i]
        ...
        sample_i = {"ref_attr_item": ref_attr_item_list_i, "ref_item": ref_item_list_i, "ref_attr_user": ref_attr_user_list_i, "ref_user": ref_user_list_i , "user": user_i,  "item": item_i, "pos_target": pos_target_i, "pos_len": pos_len_i, "neg_target": neg_target_i, "neg_len": neg_len_i, "max_seq_len": self.m_max_seq_len}
    ...
    def collate(batch):
        ...
        for i in range(batch_size):
            ...
            ref_attr_user_i = sample_i["ref_attr_user"]
            for j in ref_attr_user_i:
                ref_attr_len_user_iter.append(len(j))
            
            ref_user_i = sample_i["ref_user"]
            ref_user_len = len(ref_user_i)
            ref_user_len_iter.append(ref_user_len)

            assert len(ref_item_i) == len(ref_attr_item_i)
            assert len(ref_user_i) == len(ref_attr_user_i)
        ...
        max_ref_attr_len_user = max(ref_attr_len_user_iter)
        ...
        for i in range(batch_size):
            ...
            ref_attr_user_i = copy.deepcopy(sample_i["ref_attr_user"])

            for j in ref_attr_user_i :
                len_j = len(j)
                j.extend([freq_pad_id]*(max_ref_attr_len_user-len_j))
                ref_attr_user_iter.append(j)    
        ...
        ref_attr_user_iter_tensor = torch.from_numpy(np.array(ref_attr_user_iter)).long()
        ref_attr_len_user_iter_tensor = torch.from_numpy(np.array(ref_attr_len_user_iter)).long()
        ...
        ref_user_len_iter_tensor = torch.from_numpy(np.array(ref_user_len_iter)).long()
        ...
        return ref_attr_item_iter_tensor, ref_attr_len_item_iter_tensor, ref_item_len_iter_tensor, ref_attr_user_iter_tensor, ref_attr_len_user_iter_tensor, ref_user_len_iter_tensor, user_iter_tensor, item_iter_tensor, pos_target_iter_tensor, pos_len_iter_tensor, neg_target_iter_tensor, neg_len_iter_tensor
...
class _WINE_TEST(Dataset):
    def __init__(self, args, vocab_obj, train_df, df):
        ...
        user_itemdict_attrlist_dict, item_userdict_attrlist_dict = self.f_split(train_df)
    ...
    def __getitem__(self, idx):
        ...
        sample_i = {"ref_attr_item": ref_attr_item_list_i, "ref_item": ref_item_list_i,  "ref_attr_user": ref_attr_user_list_i, "ref_user": ref_user_list_i, "user": user_i,  "item": item_i,  "target": target_i, "target_len": target_len_i, "max_seq_length": self.m_max_seq_len}
    ...
    def collate(batch):
        ...
        return  ref_attr_item_iter_tensor, ref_attr_len_item_iter_tensor, ref_item_len_iter_tensor,ref_attr_user_iter_tensor, ref_attr_len_user_iter_tensor, ref_user_len_iter_tensor, user_iter_tensor, item_iter_tensor, target_iter_tensor, target_mask_iter_tensor
