...
class ATTR_TRANSFORMER(nn.Module):
    ...
    def forward(self, x, x_mask, user, user_size):
        ...
        # # print("acc_user_size", acc_user_size)
        # attn_weighted_x = []
        # attn_weight = []
        # for i, size_i in enumerate(user_size):
            
        #     ### embed_size
        #     attn_user_i = attn_user[i]
        #     acc_user_size_i = acc_user_size[i]
        #     x_i = x[last_size: acc_user_size_i]
        #     attn_x_i = attn_x[last_size: acc_user_size_i]
        #     x_mask_i = x_mask[last_size: acc_user_size_i]

        #     ### user_size*embed_size
        #     dup_user = attn_user_i.repeat(size_i, 1)

        #     last_size = acc_user_size_i

        #     ### user: batch_size*hidden_size*1
        #     ### attn_weight: batch_size*set_size

        #     # print("attn_x_i", attn_x_i.size())
        #     # print("x_mask_i", x_mask_i.size())
        #     # print("dup_user", dup_user.size())

        #     attn_weight_i = self.m_softmax(torch.matmul(attn_x_i*x_mask_i.unsqueeze(-1), dup_user.unsqueeze(-1)).squeeze(-1))

        #     ### attn_weighted_x: batch_size*set_size*dim_hidden
        #     attn_weighted_x_i = torch.sum(attn_x_i*(attn_weight_i*x_mask_i).unsqueeze(-1), dim=1)

        #     attn_weighted_x.append(attn_weighted_x_i)

        #     # print(attn_weighted_x_i.size())
        #     attn_weight.append(attn_weight_i)

        # attn_weighted_x = torch.cat(attn_weighted_x, dim=0)
