...
from flicker import FLICKER, FLICKER_TEST
...
class _DATA():
    ...
    def f_load_data_wine(self, args):
        ...
        train_data = WINE(args, vocab_obj, train_df, item_boa_dict, user_boa_dict)
        valid_data = WINE_TEST(args, vocab_obj, valid_df, item_boa_dict, user_boa_dict)
    ...
    def f_load_data_flicker(self, args):
        self.m_data_name = args.data_name
        # self.m_vocab_file = self.m_data_name+".vocab.json"
        self.m_item_boa_file = args.item_boa_file
        self.m_user_boa_file = args.user_boa_file

        train_data = FLICKER(args)
        valid_data = FLICKER_TEST(args)
        ...
        if args.parallel:
            ...
            train_loader = DataLoader(dataset=train_data, batch_size=batch_size, shuffle=False, num_workers=4, collate_fn=train_data.collate)
        test_loader = DataLoader(dataset=valid_data, batch_size=12305, shuffle=False, num_workers=0, collate_fn=valid_data.collate)
