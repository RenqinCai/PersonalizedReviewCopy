...
class _TRAINER(object):
    ...
    def f_train_epoch(self, train_data, network, optimizer, logger_obj, epoch_index):
        ...
        for x_batch, y_batch in train_data:		
            ...
            x_gpu = x_batch.to(self.m_device)
            y_gpu = y_batch.to(self.m_device)

            enc_x, enc_y, logits = network(x_gpu, y_gpu, batch_index, epoch_index)
            ...
            loss_list.append(loss.item()/x_batch.size(0))
            precision_list.append(precision)
            recall_list.append(recall)

            tmp_loss_list.append(loss.item()/x_batch.size(0))
    ...
    def f_eval_epoch(self, eval_data, network, optimizer, logger_obj, epoch_index):
        ...
        gamma = 0.001
        with torch.no_grad():
            for x_batch, y_batch, y_mask_batch in eval_data:		
                ...
                logits = network.f_eval_forward(x_gpu, epoch_index)
