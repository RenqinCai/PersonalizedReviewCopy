...
from torch.utils.data import DataLoader
from metric import _REC_LOSS, _KL_LOSS_CUSTOMIZE, _KL_LOSS_STANDARD, _BOW_LOSS
...
class _TRAINER(object):
	...
	def f_train_en_epoch(self, train_data, network, en_optimizer, logger_obj):
		...
		network.train()
		for input_batch, input_bow_batch, input_length_batch, user_batch, item_batch, target_batch, target_l_batch, target_length_batch, random_flag in train_data:
			...
			# print("+++"*20)
			user_logits, item_logits = network(input_bow_batch_gpu, user_batch_gpu, item_batch_gpu)
			...
			user_en_NLL_loss = self.m_bow_loss(user_logits, input_bow_batch_gpu)
			en_NLL_loss = user_en_NLL_loss/batch_size

			item_en_NLL_loss = self.m_bow_loss(item_logits, input_bow_batch_gpu)
	...
	def f_eval_en_epoch(self, eval_data, network, en_optimizer, logger_obj):
		...
		with torch.no_grad():
			for input_batch, input_bow_batch, input_length_batch, user_batch, item_batch, target_batch, target_l_batch, target_length_batch, random_flag in eval_data:
				...
				user_en_NLL_loss = self.m_bow_loss(user_logits, input_bow_batch_gpu)
				en_NLL_loss = user_en_NLL_loss

				item_en_NLL_loss = self.m_bow_loss(item_logits, input_bow_batch_gpu)
				en_NLL_loss += item_en_NLL_loss
	...
	def f_train_de_epoch(self, train_data, network, de_optimizer, logger_obj):
		...
		for input_batch, input_bow_batch, input_length_batch, user_batch, item_batch, target_batch, target_l_batch, target_length_batch, random_flag in train_data:
			...
			logits, l_logits, bow_logits, mu_z, logvar_z, mu_p_z, logvar_p_z = network(input_batch_gpu, input_bow_batch_gpu, target_l_batch_gpu[:, :-1], user_batch_gpu, item_batch_gpu, random_flag)

			# print("logits size: ", logits.size())
			# print("target_batch_gpu[:, 1:] size: ", target_batch_gpu[:, 1:].size())
			...
			gen_l_NLL_loss = self.m_rec_l_loss(l_logits, target_l_batch_gpu[:, 1:], target_length_batch-1)

			gen_BOW_loss = self.m_bow_loss(bow_logits, input_bow_batch_gpu)
			
			KL_loss = self.m_kl_loss_z(mu_z, logvar_z, mu_p_z, logvar_p_z)

			loss = gen_NLL_loss+gen_BOW_loss+KL_loss/batch_size+gen_l_NLL_loss/batch_size
	...
	def f_eval_de_epoch(self, eval_data, network, de_optimizer, logger_obj):
		...
		with torch.no_grad():
			for input_batch, input_bow_batch, input_length_batch, user_batch, item_batch, target_batch, target_l_batch, target_length_batch, random_flag in eval_data:
				...
				loss = gen_NLL_loss+gen_BOW_loss+KL_loss/batch_size
