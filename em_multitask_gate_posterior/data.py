...
class _DATA():
    ...
    def f_load_data_cloth(self, args):
        ...
        valid_data = _CLOTH_TEST(args, vocab_obj, valid_df)
    ...
    def f_load_data_yelp(self, args):
        ...
        corpus_stop_word_list = ['<SOS>', '<EOS>', '<UNK>', 'food', 'friendly', 'good', 'great', 'just', 'nice', 'service']
        stop_word_list = stop_word_list + corpus_stop_word_list

        vocab_obj.f_set_user(global_user2iid)
        vocab_obj.f_set_item(global_item2iid)

        vocab_obj.f_set_stopwords(stop_word_list)
        ...
        vocab_obj.f_set_cont_vocab(cont_w2i, cont_i2w)

        vocab_obj.f_set_func_vocab(func_w2i, func_i2w)
...
class _Vocab():
    def __init__(self):
        ...
        self.m_stop_words = []
        self.m_stop_word_ids = []

        self.m_cont_w2i = None
        self.m_cont_i2w = None
        self.m_cont_vocab_size = 0

        self.m_func_w2i = None
        self.m_func_i2w = None
        self.m_func_vocab_size = 0

    def f_set_stopwords(self, stop_words):
        for word in stop_words:
            if word not in self.m_w2i:
                continue
            wid = self.m_w2i[word]
            self.m_stop_words.append(word)
            self.m_stop_word_ids.append(wid)

    def f_set_cont_vocab(self, w2i, i2w):
        self.m_cont_w2i = w2i
        self.m_cont_i2w = i2w
        self.m_cont_vocab_size = len(w2i)

    def f_set_func_vocab(self, w2i, i2w):
        self.m_func_w2i = w2i
        self.m_func_i2w = i2w
        self.m_func_vocab_size = len(w2i)
