...
def main(args):
    ...
    ### get model
    network = _ENC_NETWORK(vocab_obj, args)
    ...
    if args.train:
        ...
        if args.parallel:
            trainer.f_train_E(train_data, valid_data, network, optimizer, logger_obj, local_rank)
        else:
            trainer.f_train_E(train_data, valid_data, network, optimizer, logger_obj, 0)
    ...
    if args.load:
        ...
        args.model_file = E_model_abs_file

        trainer = _TRAINER(vocab_obj, args, device)
        trainer.f_get_user_item(train_data, network, logger_obj)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-nl', '--layers_num', type=int, default=1)
