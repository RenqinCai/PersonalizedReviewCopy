...
class _ATTR_NETWORK(nn.Module):
    ...
    def forward(self, attr_item, attr_tf_item, attr_lens_item, item_ids, attr_user, attr_tf_user, attr_lens_user, user_ids):
        ...
        item_output = item_embed+user_attr_item_output
        user_output = user_embed+item_attr_user_output
        ...
        # user_output = torch.cat([user_embed, item_attr_user_output], dim=1)
        # item_output = torch.cat([item_embed, user_attr_item_output], dim=1)
    ...
    def f_eval_forward(self, output):

        item_attr_user = torch.matmul(output, self.m_output_attr_embedding.weight.t())

        return item_attr_user