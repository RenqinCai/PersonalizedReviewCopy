...
from parallel_model import _ATTR_NETWORK
...
def main(args):
    ...
    if args.train:
        ...
        if args.parallel:
            network = torch.nn.parallel.DistributedDataParallel(network, device_ids=[local_rank], output_device=local_rank, find_unused_parameters=False)

        optimizer = _OPTIM(network.parameters(), args)
        trainer = _TRAINER(vocab_obj, args, device)
        trainer.f_train(pretrain_word_embed, train_data, valid_data, network, optimizer, logger_obj, local_rank)
