...
class _ATTR_NETWORK(nn.Module):
    ...
    def forward(self, attr_item, attr_tf_item, attr_lens_item, item_ids, attr_user, attr_tf_user, attr_lens_user, user_ids, pos_targets, pos_lens, neg_targets, neg_lens):
        ...
        output_embed = torch.cat([user_embed, item_embed], dim=-1)
        output_attr = torch.cat([item_attr_user_output, user_attr_item_output], dim=-1)
        output_attr = self.m_attr_output(output_attr)

        output = (1-self.m_gamma)*output_embed+self.m_gamma*output_attr

        neg_embed = self.m_output_attr_embedding(neg_targets)
        neg_logits = self.f_get_logits(neg_embed, output)
        ...
        pos_logits = self.f_get_logits(pos_embed, output)
