...
class _EVAL(object):
    ...
    def f_eval_new_user(self, train_data, eval_data):
        ...
        with torch.no_grad():
            for input_batch, input_length_batch, user_batch, item_batch, target_batch in eval_data:
                ...
                # print("here")
                precision_batch, recall_batch = self.f_eval_bow_user(user_batch, input_batch, target_batch, k=1)
    ...
    def f_eval_bow_user(self, users, inputs, targets, k=10):
        ...
        for sample_i in range(batch_size):
            ...
            user_boa = self.m_user_boa_dict[str(userid_i)][0][:k]
            user_boa_freq = self.m_user_boa_dict[str(userid_i)][1][:k]
            ...
            pred_i = (pred_freq_i > 0)
            target_i = targets[sample_i]
            true_pred = (pred_i == target_i)*(target_i==1)
            if torch.sum(true_pred) > 0:
                print("user id", userid_i)
                print("--"*10, "preds", "--"*10)
                for j, pred_i_j in enumerate(pred_i):
                    if pred_i_j == 0:
                        continue
                    else:
                        print(input_i[j].item(), self.m_i2w[str(input_i[j].item())], pred_freq_i[j].item(), end=" ")
                print()
                print("--"*10, "target", "--"*10)
                for j, pred_i_j in enumerate(target_i):
                    if pred_i_j == 0:
                        continue
                    else:
                        print(self.m_i2w[str(input_i[j].item())], end=" ")
                print()
        
        true_pos = (preds == targets)*(targets==1)
        true_pos = torch.sum(true_pos, dim=1)
        true_pos = true_pos.float()
