...
class INFER(object):
    ...
    def f_inference(self, eval_data):
        ...
        for input_batch, target_batch, length_batch in eval_data:
            ...
            # mean = torch.randn(hidden.size()).to(self.m_device)
            # mean = hidden
            max_seq_len = max(length_batch)
            samples, scores = self.f_decode_text(last_en_hidden, hidden_0, max_seq_len)
    ...
    def f_decode_text(self, z, hidden_0, max_seq_len, n=4):
        ...
        init_hidden = torch.zeros(z.size()).to(self.m_device)

        hidden = init_hidden.unsqueeze(0)
        ...
        while(t < max_seq_len and len(running_seqs)>0):
            ...
            # print("hidden_0 ", hidden_0.size())
            # print("repeat_hidden_0 ", repeat_hidden_0.size())
            repeat_hidden_0 = hidden_0.unsqueeze(1).expand(output.size())
            
            # attn_weights = F.softmax(alignment_scores, dim=-1)
            # context_vector = torch.bmm(attn_weights, encoder_outputs)

            output = torch.cat([output, repeat_hidden_0], dim=-1)
