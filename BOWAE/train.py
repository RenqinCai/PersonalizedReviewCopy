...
from metric import Reconstruction_loss, KL_loss_standard, KL_loss_customize, BOW_loss
...
class TRAINER(object):
    ...
    def __init__(self, vocab_obj, args, device):
        ...
        self.m_BOW_loss_fn = BOW_loss(device=self.m_device)
    ...
    def f_train_epoch(self, data, network, optimizer, logger_obj, train_val_flag):
        ...
        for input_batch, target_batch, length_batch in data:
            ...
            logits, z_mean, z_logv, z, _ = network(input_batch, length_batch)

            # z_mean.register_hook(self.f_save_grad('z_mean'))
            # z.register_hook(self.f_save_grad('z'))

            ### NLL loss
            BOW_loss = self.m_BOW_loss_fn(logits, target_batch)
            # print("BOW loss", BOW_loss)
            BOW_loss = BOW_loss/batch_size
            ...
            iteration += 1 
            if iteration % 400 == 0:
                print("%04d, Loss %9.4f, NLL-Loss %9.4f"
                    %(iteration, loss.item(), BOW_loss.item()))
            ...
            NLL_loss_list.append(BOW_loss.item())
